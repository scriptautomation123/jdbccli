...
@SuppressWarnings("unused") // Picocli uses reflection to inject fields and instantiate commands
public class ExecSqlCmd extends BaseDatabaseCliCommand {
    ...
    @Option(
        names = "--script", 
        description = "SQL script file path"
    )
    ...
    @Option(
        names = "--params", 
        description = "SQL parameters (value1,value2,value3)"
    )
    ...
    /**
     * Constructs a new ExecSqlCmd with SQL execution service. 
     * Initializes service with password resolver for vault authentication.
     */
    public ExecSqlCmd() {
        super();
        this.service = new SqlExecutorService(
    ...
    public Integer call() {
        try {
            ...
            return ExceptionUtils.handleCliException(
                e, "execute SQL", System.err); // NOSONAR
        }
    }

    /**
     * Builds a SQL request from command-line arguments.
     * Builds a SQL request from command-line arguments.
     *
