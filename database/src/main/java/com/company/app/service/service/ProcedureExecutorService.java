...
public final class ProcedureExecutorService {
    ...
    public ExecutionResult execute(final DbRequest request) {
        ...
        return executionContext.executeWithPasswordResolution(
            request, 
            conn -> executeWithConnection(procedureRequest, conn));
    }

    /**
     * Executes procedure-specific logic with an established connection.
     ...
     * @throws SQLException if execution fails
     */
    private ExecutionResult executeProcedure(
            final ProcedureRequest request, 
            final Connection conn) throws SQLException {
        
        final Map<String, Object> validationResult = validateProcedureParameters(
            conn, request.procedure().orElse(null));
    ...
    private Map<String, Object> executeProcedureInternal(
        ...
        final Map<String, Object> result = PROCEDURE_EXECUTOR.executeProcedureWithStrings(
            conn, procedure, input, output);
