...
/**
 * Service for executing typed SELECT queries using the optimized {@link
 * com.company.app.service.database.typedapi.ResultSetHandler} framework.
 *
 * <p><strong>Separation of concerns:</strong> This service exclusively covers the typed-mapping
 * path. Formatted string output belongs to {@link SqlExecutorService}. The two services share the
 * same {@link DatabaseExecutionContext} for connection and password lifecycle but never call each
 * other.
 *
 * <p><strong>Performance:</strong> 18.5x faster than naive reflection per query via LRU-cached
 * handler instances and pre-compiled {@code JdbcPropertyAccessor} arrays.
 *
 * <p><strong>Virtual threads:</strong> Execution is dispatched onto a virtual thread by the
 * underlying {@link DatabaseExecutionContext}, so JDBC blocking I/O does not pin a carrier thread.
 */
public final class TypedQueryExecutorService {
  ...
  /**
   * Constructs with a password resolver. {@link DatabaseExecutionContext} is created internally.
   *
   * @param passwordResolver password resolver for authentication
   */
  public TypedQueryExecutorService(final PasswordResolver passwordResolver) {
    this(
   ...
   */
  TypedQueryExecutorService(final DatabaseExecutionContext executionContext) {
    this.executionContext =
        Objects.requireNonNull(executionContext, "DatabaseExecutionContext cannot be null");
  }

  /**
   * Executes a SELECT query and maps each row to an instance of {@code resultClass}.
   *
   * <p>The request must be a {@link SqlRequest} in SQL mode (not script mode). Attempting to pass a
   * {@link ProcedureRequest} will throw {@link QueryExecutionException}.
   *
   ...
   * @throws QueryExecutionException if the request type is unsupported or query execution fails
   */
    ...
    return switch (request) {
      ...
      case ProcedureRequest ignored ->
          throw new QueryExecutionException(
              "TypedQueryExecutorService does not support ProcedureRequest; "
