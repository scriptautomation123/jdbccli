...
/**
 * Facade for the typed-object mapping API. All methods return Java bean
 * instances populated from
 * JDBC {@code ResultSet} data via the optimized {@link
 * com.company.app.typedapi.ResultSetHandler} framework.
 *
 * <p>
 * <strong>Separation of concerns:</strong> This facade exclusively delegates to
 * {@link
 * TypedSqlExecutorService}. For human-readable CLI output use
 * {@link JdbcCliStringApi}.
 *
 * <p>
 * Obtain an instance via {@link JdbcCliLibrary#typed()}.
 */
public final class JdbcCliTypedApi {

  private final TypedSqlExecutorService typedQueryExecutorService;
   ...
   */
  JdbcCliTypedApi(final TypedSqlExecutorService typedQueryExecutorService) {
    this.typedQueryExecutorService = Objects.requireNonNull(
        typedQueryExecutorService, "TypedSqlExecutorService cannot be null");
  }
   ...
   *
   * <p>
   * Column names are mapped to bean setter names using underscore-to-camelCase
   * conversion (e.g.,
   ...
   * @param vaultConfig vault configuration for password resolution; {@code null}
   *                    for none
   ...
   *
   * <p>
   * Returns {@code null} when no rows match. Throws {@link IllegalStateException}
   * when more than
   * one row is returned — use
   * {@link #runSqlTypedApi(String, String, String, String, List, Class,
  ...
  public <T> T runSqlSingleTypedApi(
      ...
      final Class<T> resultClass,
      final VaultConfig vaultConfig) {
    final List<T> results = runSqlTypedApi(dbType, database, user, sql, params, resultClass, vaultConfig);
