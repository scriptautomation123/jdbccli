...
/**
 * Immutable request object for password resolution operations. Supports both vault lookup mode
 * (user/database only) and direct vault parameter mode. Validates that vault parameters are either
 * all provided or none provided.
 *
...
public record PasswordRequest(
    String user, String database, String vaultUrl, String roleId, String secretId, String ait) {
  ...
  private static final String NO_VAULT_PARAMS_MESSAGE =
      "Vault parameters not available - direct vault params not provided";
  ...
  private static boolean hasAllDirectVaultParams(
      final String vaultUrl, final String roleId, final String secretId, final String ait) {
    return StringUtils.isNotBlank(vaultUrl)
        && StringUtils.isNotBlank(roleId)
        && StringUtils.isNotBlank(secretId)
        && StringUtils.isNotBlank(ait);
  }
  ...
  /**
   * Gets the application identifier token if direct vault parameters are provided.
   *
   * @return the AIT
   * @throws IllegalStateException if direct vault parameters are not provided
   */
  ...
  private void validateDirectVaultParams(final String operation) {
    if (!hasDirectVaultParams()) {
      LoggingUtils.logStructuredError(
          ERROR_CONTEXT, operation, NO_VAULT_PARAMS_ERROR, NO_VAULT_PARAMS_MESSAGE, null);
